...
| Task Orchestration | `producer` | 对话需求与约束 | 可执行任务条目（写入 `TASK_QUEUE.md`） | 建议在 `/plan` 模式下创建任务，控制复杂度 |
| Task Orchestration | `consumer` | `TASK_QUEUE.md` 中的待办任务 | 闭环执行并归档到 `TASK_RECORD.md` | 由系统自动拉起后台监听，空闲一段时间后自动退出 |
